...
# Format and set input directory for L1B geolocaiton files
if args.geodir[-1] != '/' and args.geodir[-1] != '\\':
    geoinDir = args.geodir.strip("'").strip('"') + os.sep
else:
    geoinDir = args.geodir

# Find input directory
try:
    os.path.exists(geoinDir)
except FileNotFoundError:
...
geoList = [geoinDir + f for f in os.listdir(geoinDir) if f.endswith('.h5') and 'GEO' in f]
ecoList = [f for f in os.listdir(inDir) if f.endswith('.h5') and 'GEO' not in f]
...
# ---------------------------------IMPORT GEOLOCATION FILE--------------------------------------- #
        ...
        if len(geo) != 0 or 'L1B_MAP' in e:         # Proceed if GEO/MAP file
            if 'L1B_MAP' in e:
                ...
                g = h5py.File(geo[0])               # Read in GEO file
            geo_objs = []
